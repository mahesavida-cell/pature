...
export default function EditorsChoicePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Filter only editors choice for initial view
  ...
  const filteredAndSortedPosts = useMemo(() => {
    ...
    result.sort((a, b) => {
      ...
      if (sortBy === 'alphabet') return sortOrder === 'desc' ? b.title.localeCompare(a.title) : a.title.localeCompare(b.title);
  ...
  const groupedByAuthor = useMemo(() => {
    ...
    posts.forEach(post => {
      const author = post.author || "Redaksi PatureNews";
  ...
  if (isLoading) {
    return (
      <Container className="py-20 text-center flex flex-col items-center gap-4">
        ...
        <TypographyMuted className="text-xs" casing="sentence">Mempersiapkan pilihan redaksi...</TypographyMuted>
      </Container>
    );
  }
  ...
  return (
    <Container className="space-y-6 pt-4">
      <header className="max-w-4xl">
        <TypographyMuted className="mb-1" casing="sentence">Kurasi eksklusif</TypographyMuted>
        <Title className="text-2xl md:text-3xl leading-none mb-3">{formatCasing("Pilihan redaksi", 'sentence')}</Title>
        <TypographyMuted className="text-sm md:text-base leading-relaxed max-w-2xl" casing="sentence">
          Esei dan laporan mendalam yang dikurasi secara mandiri untuk memberikan wawasan yang melampaui berita harian.
        </TypographyMuted>
      </header>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start pt-2">
        <div className="lg:col-span-8 space-y-4">
          <TypographyLabel className="m-0" casing="sentence">Sorotan utama</TypographyLabel>
          {heroTrendingPosts.length > 0 ? (
            <Carousel opts={{ loop: true }} plugins={[Autoplay({ delay: 5000 })]} className="w-full relative group">
                    ...
                    <Link href={`/news/${post.slug}`} className="block group/hero">
                      <div className="relative aspect-[16/9] overflow-hidden rounded-xl border border-primary/5 bg-muted">
                        ...
                        <div className="absolute bottom-0 left-0 right-0 p-8 text-white">
                          <Badge className="bg-white/95 text-primary border-none shadow-none text-[9px] font-bold mb-4 px-3 py-1">Pilihan</Badge>
              ...
              <div className="absolute right-6 bottom-6 flex gap-2 z-10">
                <CarouselPrevious className="relative translate-y-0 left-0 h-9 w-9 border-white/20 bg-white/10 backdrop-blur-md text-white hover:bg-white hover:text-primary shadow-none" />
                <CarouselNext className="relative translate-y-0 right-0 h-9 w-9 border-white/20 bg-white/10 backdrop-blur-md text-white hover:bg-white hover:text-primary shadow-none" />
              </div>
            </Carousel>
          ) : <AnimatedEmptyState message="Belum ada berita pilihan." />}
        </div>

        <div className="lg:col-span-4 space-y-4">
          <TypographyLabel className="m-0" casing="sentence">Paling relevan</TypographyLabel>
          <div className="pt-1">
            {heroTrendingPosts.length > 0 ? (
            ...
            ) : <TypographyMuted className="py-20 text-center">Data tidak tersedia.</TypographyMuted>}
          </div>
        </div>
      </div>

      <div className="pt-10 space-y-16">
        {groupedByAuthor.map((group, gIdx) => (
          <section key={`author-group-${gIdx}`} className="space-y-6">
            <div className="flex items-center justify-between border-b border-primary/5 pb-4">
              <TypographyLabel className="m-0" casing="sentence">Karya {group.name}</TypographyLabel>
            </div>
      ...
      <div className="pt-12 pb-24 space-y-10">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 p-5 rounded-xl bg-white/40 border border-primary/5">
          <div className="space-y-2">
            <TypographyLabel className="m-0" casing="sentence">Arsip kurasi</TypographyLabel>
            <div className="flex flex-wrap items-center gap-3">
              <DropdownMenu>
                ...
                <DropdownMenuContent className="w-48 p-2 rounded-xl bg-white/95 backdrop-blur-xl shadow-2xl border-primary/5">
                  <DropdownMenuItem onClick={() => setSortBy('time')} className="rounded-lg gap-3 text-xs py-2.5 px-3 font-medium cursor-pointer">Waktu upload</DropdownMenuItem>
                  <DropdownMenuItem onClick={() => setSortBy('year')} className="rounded-lg gap-3 text-xs py-2.5 px-3 font-medium cursor-pointer">Tahun</DropdownMenuItem>
                  <DropdownMenuItem onClick={() => setSortBy('alphabet')} className="rounded-lg gap-3 text-xs py-2.5 px-3 font-medium cursor-pointer">A-Z</DropdownMenuItem>
                  <DropdownMenuItem onClick={() => setSortBy('popularity')} className="rounded-lg gap-3 text-xs py-2.5 px-3 font-medium cursor-pointer">Popularitas</DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          <AnimatePresence mode="popLayout">
            {filteredAndSortedPosts.length > 0 ? filteredAndSortedPosts.map((post, idx) => (
            ...
            )) : <AnimatedEmptyState message="Tidak ada hasil yang ditemukan." />}
          </AnimatePresence>
        </div>
      </div>
    </Container>
