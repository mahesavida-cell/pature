...
export default function RecommendationsPage() {
  ...
  // Sorting states
  const [sortBy, setSortBy] = useState<'time' | 'year' | 'popularity' | 'alphabet'>('popularity');
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setPosts(trendingOnly.length > 0 ? trendingOnly : (postsData || []));
  ...
  const groupedByCategory = useMemo(() => {
    ...
    posts.forEach(post => {
      const cat = post.categories?.[0] || "Populer";
  ...
  if (isLoading) {
    return (
      <Container className="py-20 text-center flex flex-col items-center gap-4">
        ...
        <TypographyMuted className="text-xs" casing="sentence">Menganalisis tren informasi...</TypographyMuted>
      </Container>
    );
  }
  ...
  return (
    <Container className="space-y-6 pt-4">
      <header className="max-w-4xl">
        <TypographyMuted className="mb-1" casing="sentence">Smart insights</TypographyMuted>
        <Title className="text-2xl md:text-3xl leading-none mb-3">{formatCasing("Berita terpopuler", 'sentence')}</Title>
        <TypographyMuted className="text-sm md:text-base leading-relaxed max-w-2xl" casing="sentence">
          Daftar berita yang paling banyak dibicarakan dan memiliki dampak signifikan bagi komunitas pembaca PatureNews.
        </TypographyMuted>
      </header>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start pt-2">
        <div className="lg:col-span-8 space-y-4">
          <TypographyLabel className="m-0" casing="sentence">Sorotan publik</TypographyLabel>
          {heroTrendingPosts.length > 0 ? (
                    ...
                    <Link href={`/news/${post.slug}`} className="block group/hero">
                      <div className="relative aspect-[16/9] overflow-hidden rounded-xl border border-primary/5 bg-muted">
                        ...
                        <div className="absolute bottom-0 left-0 right-0 p-8 text-white">
                          <Badge className="bg-white/95 text-primary border-none shadow-none text-[9px] font-bold mb-4 px-3 py-1 flex items-center gap-1.5 w-fit">
                            <Sparkles className="h-2.5 w-2.5" /> Terpopuler
                          </Badge>
            ...
            <div className="aspect-[16/9] bg-primary/5 rounded-xl border border-dashed border-primary/10">
              <AnimatedEmptyState message="Belum ada berita terpopuler saat ini." allCategories={allCategories} />
            </div>
          )}
        </div>
        <div className="lg:col-span-4 space-y-4">
          <TypographyLabel className="m-0" casing="sentence">Urutan tren</TypographyLabel>
          <div className="pt-1">
            {heroTrendingPosts.length > 0 ? (
        ...
        {groupedByCategory.map((group, idx) => (
          <section key={`group-${idx}`} className="space-y-6">
            <div className="flex items-center justify-between border-b border-primary/5 pb-4">
              <TypographyLabel className="m-0" casing="sentence">Tren di {group.name}</TypographyLabel>
            </div>
      ...
      <div className="pt-12 pb-24 space-y-10">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 p-5 rounded-xl bg-white/40 border border-primary/5">
          <div className="flex flex-col gap-2">
            <TypographyLabel className="m-0" casing="sentence">Arsip tren</TypographyLabel>
