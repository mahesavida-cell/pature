        ...
        <div className="relative z-10 flex flex-col h-full justify-between">
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <h2 className="text-2xl font-headline font-bold leading-tight text-white">{formatCasing("Kejernihan informasi di genggaman Anda", 'sentence')}</h2>
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={i} className="flex items-center gap-3 text-xs font-medium"><span className="text-white/30">{item.icon}</span><span className="text-white/90">{formatCasing(item.text, 'sentence')}</span></div>
            ))}
          </div>
        </div>
      </div>
      <div className="flex flex-col p-8 sm:p-10 justify-center bg-white">
        <Tabs defaultValue="login" className="w-full">
          ...
          <TabsContent value="login" className="space-y-4 outline-none">
            <div className="space-y-2">
              <Label htmlFor="email" className="text-xs font-bold opacity-50 px-1 tracking-tighter">Alamat email</Label>
            ...
            <div className="space-y-2">
              <Label htmlFor="password" className="text-xs font-bold opacity-50 px-1 tracking-tighter">Kata sandi</Label>
            ...
            <Button className="w-full h-11 font-bold text-xs tracking-tight rounded-lg shadow-none" onClick={() => handleAuth('login')} disabled={isLoading}>{isLoading ? "Memproses..." : "Masuk ke akun"}</Button>
          </TabsContent>
          <TabsContent value="register" className="space-y-4 outline-none">
            <div className="space-y-2">
              <Label htmlFor="reg-email" className="text-xs font-bold opacity-50 px-1 tracking-tighter">Alamat email</Label>
            ...
            <div className="space-y-2">
              <Label htmlFor="reg-password" className="text-xs font-bold opacity-50 px-1 tracking-tighter">Kata sandi baru</Label>
              <div className="relative">
            ...
            <Button className="w-full h-11 font-bold text-xs tracking-tight rounded-lg shadow-none" onClick={() => handleAuth('register')} disabled={isLoading}>{isLoading ? "Memproses..." : "Daftar sekarang"}</Button>
          </TabsContent>
        </Tabs>
        <div className="flex items-center my-6 gap-3"><div className="h-[1px] flex-1 bg-primary/5" /><span className="text-[10px] font-bold text-muted-foreground/30 tracking-widest">Atau</span><div className="h-[1px] flex-1 bg-primary/5" /></div>
        <Button variant="outline" className="w-full h-11 font-bold text-xs border border-primary/10 bg-white hover:bg-primary/5 rounded-lg shadow-none tracking-widest" onClick={handleGoogleSignIn}>GOOGLE</Button>
      </div>
    </div>
  );
}
