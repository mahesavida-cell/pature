      ...
      <div className="bg-background min-h-screen flex flex-col">
        ...
        <main className="flex-1 flex flex-col items-center justify-center p-4 text-center pt-24">
          <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }}>
            ...
            <Link href="/auth"><Button className="rounded-lg px-10 h-11 font-bold tracking-widest shadow-none">Masuk sekarang</Button></Link>
          </motion.div>
        </main>
      </div>
    );
      ...
      <main className="max-w-7xl mx-auto px-4 pt-24 md:pt-32">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          <aside className="lg:col-span-4 space-y-8">
            <Card className="rounded-xl border border-primary/5 shadow-none bg-white/40 backdrop-blur-xl">
              <CardContent className="p-8 text-center">
                <div className="relative inline-block mb-6">
                  <Avatar className="h-24 w-24 border border-white shadow-none">
            ...
            <Card className="rounded-xl bg-primary text-primary-foreground p-8 shadow-none relative overflow-hidden group border-none">
              ...
              <div className="relative z-10">
                ...
                <div className="space-y-4">
                  {recommendations.length > 0 ? recommendations.map((rec, idx) => (
                    <motion.div key={rec.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.1 }} className="p-4 rounded-lg bg-white/5 hover:bg-white/10 border border-white/5 transition-all cursor-pointer group/item shadow-none">
          ...
          <section className="lg:col-span-8">
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              <TabsList className="bg-transparent border-b border-primary/5 rounded-none w-full justify-start h-auto p-0 mb-6 space-x-12 shadow-none">
                <TabsTrigger value="editor" className="bg-transparent rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent px-0 py-4 text-[10px] font-bold tracking-widest transition-all shadow-none">Editor akun</TabsTrigger>
                <TabsTrigger value="archived" className="bg-transparent rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent px-0 py-4 text-[10px] font-bold tracking-widest transition-all shadow-none">Berita diarsipkan</TabsTrigger>
                <TabsTrigger value="history" className="bg-transparent rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent px-0 py-4 text-[10px] font-bold tracking-widest transition-all shadow-none">Riwayat bacaan</TabsTrigger>
              </TabsList>
                    ...
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
                      <Card className="rounded-xl p-10 bg-white/60 backdrop-blur-md border border-primary/5 shadow-none">
                        <div className="space-y-8">
                          ...
                          <Button onClick={handleUpdateProfile} className="w-full h-12 rounded-lg font-bold text-[11px] tracking-widest shadow-none" disabled={isUpdating}>
                    ...
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {isBookmarksLoading ? (
                        ...
                        <motion.div key={item.id} initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: idx * 0.05 }}>
                          <Card className="rounded-xl border border-primary/5 shadow-none hover:border-primary/20 transition-all group h-full bg-white/60 backdrop-blur-md">
                            <CardContent className="p-7 flex flex-col justify-between h-full">
                              <div>
                                <Badge variant="secondary" className="text-[8px] font-bold mb-4 bg-primary/5 text-primary border-none shadow-none uppercase tracking-wider">{item.category}</Badge>
                      ...
                      {isHistoryLoading ? (
                        ...
                        <motion.div key={item.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.05 }}>
                          <Link href={`/news/${item.postId}`}>
                            <div className="flex items-center justify-between p-7 rounded-xl hover:bg-white/80 transition-all group bg-white/40 backdrop-blur-xl border border-primary/5 shadow-none">
                              <div className="flex items-center gap-6">
                                <div className="h-10 w-10 rounded-lg bg-primary/5 flex items-center justify-center text-primary/40 shadow-none shrink-0"><LayoutDashboard className="h-5 w-5" /></div>
