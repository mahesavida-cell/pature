...
import { useUser, useFirestore, useCollection, useDoc, useMemoFirebase, addDocumentNonBlocking, updateDocumentNonBlocking, setDocumentNonBlocking, deleteDocumentNonBlocking } from "@/firebase";
...
const ShareButton = ({ post }: { post: any }) => {
  ...
  useEffect(() => { setUrl(window.location.href); setCanShare(typeof navigator !== 'undefined' && !!navigator.share); }, []);
  const handleShare = async () => { if (canShare) { try { await navigator.share({ title: post.title, text: post.excerpt, url }); } catch (e) {} } };
  const copyToClipboard = () => { navigator.clipboard.writeText(url); toast({ title: "Tautan disalin", description: "Tautan telah disalin ke papan klip." }); };
  if (canShare) return <Button variant="outline" size="icon" className="rounded-full h-9 w-9 border-primary/10 bg-white/40 shadow-none" onClick={handleShare}><Share2 className="h-4 w-4" /></Button>;
  return (
    <Popover>
    ...
    <div className={cn("space-y-4", depth > 0 && "ml-6 md:ml-10 border-l-2 border-primary/5 pl-4 md:pl-6")}>
      ...
      {replyToId === comment.id && (
        <div className="ml-6 space-y-3">
          ...
          <div className="flex justify-end gap-2"><Button variant="ghost" size="sm" onClick={() => setReplyToId(null)} className="h-8 text-[10px] font-bold uppercase">Batal</Button><Button size="sm" onClick={() => onReply(comment.id)} className="h-8 text-[10px] font-bold shadow-none uppercase">Kirim</Button></div>
        </div>
      )}
...
export default function NewsDetailPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (!params?.id) return;
      try {
        const [postData, trendingData] = await Promise.all([client.fetch(POST_DETAIL_QUERY, { slug: params.id }), client.fetch(TRENDING_POSTS_QUERY)]);
        setSanityPost(postData); setTrendingPosts(trendingData || []);
  ...
  useEffect(() => {
    if (user && db && params?.id && sanityPost) {
      ...
      setDocumentNonBlocking(historyRef, { postId: params.id, title: sanityPost.title, category: sanityPost.categories?.[0] || "Berita", viewedAt: new Date().toISOString() }, { merge: true });
    }
  ...
  const threadedComments = useMemo(() => {
    ...
    const map = new Map(); firestoreComments.forEach(c => map.set(c.id, { ...c, replies: [] }));
    const roots: any[] = []; firestoreComments.forEach(c => { const item = map.get(c.id); if (c.parentId && map.has(c.parentId)) map.get(c.parentId).replies.push(item); else roots.push(item); });
    return roots;
  }, [firestoreComments]);
  ...
  const handleToggleBookmark = () => {
    ...
    if (isSaved) { deleteDocumentNonBlocking(bookmarkRef); toast({ title: "Dihapus", description: "Dihapus dari arsip." }); }
    else { setDocumentNonBlocking(bookmarkRef, { postId: params?.id, title: sanityPost.title, category: sanityPost.categories?.[0] || "Berita", savedAt: new Date().toISOString() }, { merge: true }); toast({ title: "Berhasil diarsipkan", description: "Tersimpan di profil." }); }
  };
  ...
  const handlePostComment = (parentId: string | null = null) => {
    ...
    if (!text.trim()) return;
    addDocumentNonBlocking(collection(db, "posts", params.id as string, "comments"), { content: text, authorId: user.uid, authorName: user.displayName || user.email?.split('@')[0] || "Pengguna PatureNews", createdAt: serverTimestamp(), postId: params.id, parentId, likes: [] });
